...
class DebugHandler(BaseMiddleware):
    ...
    async def send(
        ...
        for key, value in response.headers.items():
            print(f"{key}: {value}")
        if "Content-Type" in response.headers and response.headers["Content-Type"] == "application/json" and "Content-Length" in response.headers and int(response.headers["Content-Length"]) > 0:
            print("")
            print("Response body:")
